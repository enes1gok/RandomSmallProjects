...
print("---- "+ "----- "+ "------------ "+ "---- "+ "----")
for j, y, l, z, k in zip(sıra_liste, lno_liste, ad_soyad_liste, elo_liste, ukd_liste):
...
while True:
    tur_sayısı = int(input("Turnuvadaki tur sayisini giriniz ({}-{}): ".format(new_x, sıra-2)))
    if new_x <= tur_sayısı <= sıra - 2:
        break
    else:
        continue
...
class Oyuncu(): #Her oyuncu için niteliklerinin bir sınıf içerisinde toplanması
    def __init__(self,masa_no, bsno, lno, puan, renkler, dogruluk):
        ...
        for i in range(0,len(puan_liste)):
            for j in range(i+1,len(puan_liste)):
                if (sıra-1) % 2 == 1:
                    ...
                    if lno_liste[j] not in tum_karsılasmalar[i]:
                        ...
                        for k in range(0,len(lno_liste)):
                            ...
                            for m in range(k+1,len(lno_liste)):
                                if lno_liste[m] not in tum_karsılasmalar[k]:
                                    if ad_soyad_liste[m].dogrulugu == True:                                      
                                        if ad_soyad_liste[k].puanı-ad_soyad_liste[m].puanı == min(fark):
                                            sayac += 1
                                            if sayac == 1:
                                                if ad_soyad_liste[k].dogrulugu == True:
                                                    if ad_soyad_liste[m].dogrulugu == True:
                                                        ...
                                                        print(f"{lno_liste[k]} ile {lno_liste[m]} eşleştirildi")
                                                        sayac += 1
                                                        break                                  
                                        else:
                                            continue
                    ...
                    if lno_liste[j] not in tum_karsılasmalar[i]: #Eğer önceden eşleşmemişse koşula girsin.Herkesin farklı kişilerle eşleşmesinden dolayı
                        fark=[]            
                        for k in range(0,len(lno_liste)):
                            for m in range(k+1,len(lno_liste)):
                                if lno_liste[m] not in tum_karsılasmalar[k]:
...
for i in range(1,tur_sayısı+1): #Tur sayısı kadar dönerek istenilen inputları ve tabloları vermek için.
                ...
                for p in range(0,len(ad_soyad_liste)):                
                        if masa_numaraları[p] == f: #Aynı masada olan oyuncuları bulmak için
                            if ad_soyad_liste[p].dogrulugu == True and ad_soyad_liste[lno_liste.index(tum_karsılasmalar[p][i-1])].dogrulugu == True:
                                tur_beyaz_puan.append(ad_soyad_liste[p].puanı)
                                tur_siyah_puan.append(ad_soyad_liste[lno_liste.index(tum_karsılasmalar[p][i-1])].puanı)
                                ad_soyad_liste[p].dogrulugu = False
    ...
    for n in range(0,len(sıra_liste)):
        ad_soyad_liste[n].dogrulugu = True
    a = 0
    if (sıra-1) % 2 == 1: #Eğer oyuncu sayısı tekse tek kalan oyuncu için yapılacak işlemde kullandım.
        for f in masa_numaraları2:
            if f == masa_numaraları2[-1]:
                if a == 0:
                    for p in range(0,len(ad_soyad_liste)):                
        ...
        for f in masa_numaraları2:
            for p in range(0,len(ad_soyad_liste)):                
                        if masa_numaraları[p] == f: #Aynı masada olan oyuncuları bulmak için
                            if ad_soyad_liste[p].dogrulugu == True and ad_soyad_liste[lno_liste.index(tum_karsılasmalar[p][i-1])].dogrulugu == True:
                                tur_beyaz_puan.append(ad_soyad_liste[p].lnosu)
                                tur_siyah_puan.append(ad_soyad_liste[lno_liste.index(tum_karsılasmalar[p][i-1])].lnosu)
                                ad_soyad_liste[p].dogrulugu = False
                                ad_soyad_liste[lno_liste.index(tum_karsılasmalar[p][i-1])].dogrulugu = False
    for n in range(0,len(sıra_liste)):
        ad_soyad_liste[n].dogrulugu = True
    ...
    if (sıra-1) % 2 == 1: #Eğer oyuncu sayısı tekse tek kalan oyuncu için yapılacak işlemde kullandım.
        ...
        for f in masa_numaraları2:
            for p in range(0,len(ad_soyad_liste)):                
                        if masa_numaraları[p] == f: #Aynı masada olan oyuncuları bulmak için
                            if ad_soyad_liste[p].dogrulugu == True and ad_soyad_liste[lno_liste.index(tum_karsılasmalar[p][i-1])].dogrulugu == True:
                                tur_beyaz_puan.append(ad_soyad_liste[p].bsnosu)
                                tur_siyah_puan.append(ad_soyad_liste[lno_liste.index(tum_karsılasmalar[p][i-1])].bsnosu)
                                ad_soyad_liste[p].dogrulugu = False
                                ad_soyad_liste[lno_liste.index(tum_karsılasmalar[p][i-1])].dogrulugu = False
    sayac5 = 0
    if (sıra-1) % 2 == 1:
    ...
    print("{}. Tur Eşleştirme Listesi:".format(i))
...
print("--- "+"---- "+ "----- "+ "------------ "+ "---- "+ "---- "+"---- "+"----- "+"----- "+"----- "+"--")
for a,b,c,d,e,f,g,h,ı,i,j in zip(sıra_liste1,sıra_liste,lno_liste,ad_soyad_liste2,elo_liste, ukd_liste,puan_liste,bh1,bh2,sb,gs):
